...
Сервис на NestJS, который решает 2 задачи:
...
Проект использует `.env.example` в `docker-compose.yml` как runtime-конфиг контейнеров.

Перед запуском заполните в `.env.example` реальные значения:
...
Важно: сервисный аккаунт должен иметь доступ `Editor` к каждой таблице из `GOOGLE_SHEETS_IDS`.

---

## 🚀 Запуск

Основной запуск одной командой:
...
3. Выполняются миграции (`npm run migrate:dev`).
...
- Endpoint: `GET https://common-api.wildberries.ru/api/v1/tariffs/box?date=YYYY-MM-DD`
...
- Берутся данные из БД за текущую дату
...
Локальный запуск `npm run migrate:dev`/`rollback:*` может не видеть `POSTGRES_*` переменные из Docker.
Поэтому миграции и откаты рекомендуется запускать внутри контейнера:
...
После готовности отправляется ссылка на GitHub-репозиторий в HH.
